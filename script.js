...
function doLogin() {
  ...
  for (const school of platformSchools) {
    ...
    // 2. Check teachers and admins registered inside this school
    const prevId = currentSchoolId;
    loadSchoolContext(school);
    ...
    // Not matched — restore and try next school
    currentSchoolId = prevId;
  }
...
function initApp() {
  // Multi-school: show platform login first; school data loads after school is picked
  initLang();
  const creds = getPlatformCreds();
  // Update platform login label for first-run vs returning
  ...
  if (!creds) {
    ...
    if (plSubtitle) plSubtitle.textContent = 'Create your master admin credentials for this platform';
